...
class Challenge:
    ...
    def _create_file(self, file):
        ...
        if template:
            ...
            if tmpl_path.is_file():
                ...
                try:
                    content = tmpl.render(
                        repo_conf=self.repo.conf, chal_conf=self.conf
                    )
                except:
    ...
    async def _run(self, script, dev, timeout):
        ...
        if script_path.is_absolute():
            ...
            if len(script_parents) > 1:
                cwd /= script_parents[0]
        app_log.info("running %s within %s.", script_path.name, cwd)
        if dev:
            proc = await create_subprocess_exec(
                script, '--dev', stdout=PIPE, stderr=PIPE, cwd=str(cwd)
            )
        else:
            proc = await create_subprocess_exec(
                script, stdout=PIPE, stderr=PIPE, cwd=str(cwd)
            )
        rcode = -1
        stdout = None
        stderr = None
        ...
        if rcode == 0:
            ...
            app_log.warning(
                "subprocess terminated unsuccessfully (rcode=%d).", rcode
            )
        return {
            'rcode': rcode,
            'stdout': stdout,
    ...
    def init(self):
        ...
        for directory in dir_list:
            if not self._create_dir(directory):
                app_log.warning("directory exists already: %s", directory)
        file_list = self.repo.conf.files(self.conf.category)
        for file in file_list:
            if not self._create_file(file):
                app_log.warning("file exists already: %s", file)
    ...
    def export(self, export_dir, include_disabled):
        ...
        if not include_disabled and not self.conf.enabled:
            app_log.warning("export ignored %s (disabled)", self.conf.slug)
            return

        app_log.info("exporting %s...", self.conf.slug)
        archive_name = self.conf.static_url.split('/')[-1]
        if not archive_name:
            app_log.error(
                "export ignored %s (invalid/empty static_url)", self.conf.slug
            )
            app_log.error(
        ...
        with tarfile.open(str(archive_path), 'w:gz') as arch:
            for directory in self.repo.conf.directories(
                self.conf.category, public_only=True
            ):
                dir_path = self.path.joinpath(directory)
                for entry in scandir(dir_path):
                    ...
                    app_log.debug("adding %s to archive...", entry_path)
                    arch.add(str(entry_path), arcname=entry.name)
            with tempfile.NamedTemporaryFile('w') as tmpfile:
                tmpfile.write(checksum_file.content)
                tmpfile.flush()
                app_log.debug("adding checksum.sha256 to archive...")
