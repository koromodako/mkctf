...
async def worker_routine(worker_id, monitor):
    ...
    while True:
        ...
        # process task
        await monitor.print(
            f"[{worker_id}]: waiting for {task.slug} to reach schedule: {naturaldelta(task.countdown)} remaining"
        )
        await task.is_ready()
        await monitor.print(
            f"[{worker_id}]: running healthcheck for {task.slug}"
        )
        try:
            ...
            await monitor.print(
                f"[{worker_id}]: an exception occured while running healthcheck for {task.slug}: {exc}"
            )
        else:
...
class MKCTFMonitor:
    ...
    async def healthcheck(self, slug):
        ...
        async for report in self._api.healthcheck(
            slug=slug, timeout=self._task_timeout
        ):
            yield report
    ...
    async def post(self, caller_id, slug, healthy):
        ...
        try:
            async with ClientSession(
                auth=self._auth, timeout=self._post_timeout
            ) as session:
                await self.print(
                    f"[{caller_id}]: posting {slug} report to {self._url}..."
                )
                async with session.post(
                    self._url, ssl=self._ssl, json={slug: healthy}
                ) as resp:
                    if resp.status < 400:
            ...
            await self.print(
                f"[{caller_id}]: an execption occured while contacting the scoreboard: {exc}"
            )
    ...
    async def run(self):
        ...
        for challenge in self._api.enum():
            if not challenge['conf']['enabled']:
                await self.print(
                    f"[monitor]: {challenge['slug']} skipped (disabled)"
                )
                continue
            await self.print(
                f"[monitor]: injecting a task for {challenge['slug']}..."
            )
            await self._task_queue.put(MonitorTask(self, challenge['slug']))
        # check if queue is empty before starting
        if self._task_queue.empty():
            await self.print("[monitor]: no task to process, exiting.")
            return
        ...
        await self.print("[monitor]: waiting for tasks to be processed...")
        try:
            await self._task_queue.join()
        except CancelledError:
            await self.print("[monitor]: tasks cancelled.")
        # terminate workers
        await self.print("[monitor]: terminating workers...")
        for _ in self._workers:
            await self._task_queue.put(None)
        # await workers termination
        await self.print("[monitor]: waiting for workers to terminate...")
        await gather(*self._workers, return_exceptions=True)
        await self.print("[monitor]: exiting.")
