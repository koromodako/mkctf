...
class ChallengeConfigurationWizard:
    ...
    def show(self):
        while True:
            # - tags & difficulties
            self._name = readline(
                "Enter challenge display name", default=self.default['name']
            )
            # consistency: keep previous slug even if challenge renamed
            self._slug = self._slug or slugify(self._name)
            self._tags = choose(
                self._repo_conf.tags, "Tags Selection", multi=True
            )
            tags_str = '\n - '.join(self._tags)
            print(f"Selected tags:\n - {tags_str}")
            # consistency: keep previous flag even if challenge renamed
            self._flag = self._flag or self._repo_conf.make_rand_flag()
            self._author = readline(
                "Enter challenge author name",
                empty=True,
                default=self.default['author'],
            )
            self._points = readline(
                "Enter challenge points or '-3' for dynamic",
                default=self.default['points'],
            )
            # self._enabled = self._enabled
            self._category = choose(
                self._repo_conf.categories, "Category Selection"
            )
            print(f"Selected category: {self._category}")
            self._logo_url = readline(
                "Enter challenge logo url", empty=True, default=self._logo_url
            )
            self._difficulty = choose(
                self._repo_conf.difficulties, "Difficulty Selection"
            )
            print(f"Selected difficulty: {self._difficulty}")
            # consistency: keep previous enabled even if challenge renamed
            self._static_url = (
                self._static_url or self._repo_conf.make_static_url(self._slug)
            )
