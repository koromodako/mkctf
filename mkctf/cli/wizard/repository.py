...
class RepositoryConfigurationWizard:
    ...
    def __init__(self, general_conf, prev_conf=None):
        if not prev_conf:
            prev_conf = {}
        self.default = {
            'name': f'Example CTF {datetime.now().year}',
            'tags': prev_conf.get('tags', general_conf.tags),
            'difficulties': prev_conf.get(
                'difficulties', general_conf.difficulties
            ),
            'flag_prefix': prev_conf.get('flag', {}).get(
                'prefix', general_conf.flag_prefix
            ),
            'flag_suffix': prev_conf.get('flag', {}).get(
                'suffix', general_conf.flag_suffix
            ),
            'domain': prev_conf.get('domain', general_conf.domain),
            'docker_user': prev_conf.get('docker', {}).get(
                'user', general_conf.docker_user
            ),
            'docker_registry': prev_conf.get('docker', {}).get(
                'registry', general_conf.docker_registry
            ),
        }
        # - name
        ...
        self._difficulties = self.default['difficulties']
        # - flag
        self._flag_prefix = self.default['flag_prefix']
        self._flag_suffix = self.default['flag_suffix']
        # - domain
        self._domain = self.default['domain']
        # - docker
        self._docker_user = self.default['docker_user']
        self._docker_registry = self.default['docker_registry']
    ...
    def show(self):
        while True:
            ...
            self._difficulties = choose(
                self.default['difficulties'],
            ...
            # - flag
            self._flag_prefix = readline(
                "Enter flag prefix", default=self.default['flag_prefix']
            )
            self._flag_suffix = readline(
                "Enter flag prefix", default=self.default['flag_suffix']
            )
            # - domain
            self._domain = readline(
                "Enter domain", default=self.default['domain']
            )
            # - docker
            self._docker_user = readline(
                "Enter docker user", default=self.default['docker_user']
            )
            self._docker_registry = readline(
                "Enter docker registry host",
                default=self.default['docker_registry'],
            )
